...
const settings = reactive({
  ...
  showNotify: $storage.configure.showNotify
});
...
/** 通知设置 */
const notifyChange = () => {
  const showNotify = settings.showNotify;
  storageConfigureChange("showNotify", showNotify);
};
  ...
  <LayPanel>
    <div class="p-5">
      ...
      <ul class="setting mt-2">
        ...
        <li>
          <span class="dark:text-white">{{ t("panel.pureShowNotify") }}</span>
          <el-switch
            v-model="settings.showNotify"
            inline-prompt
            :active-text="t('buttons.pureOpenText')"
            :inactive-text="t('buttons.pureCloseText')"
            @change="notifyChange"
